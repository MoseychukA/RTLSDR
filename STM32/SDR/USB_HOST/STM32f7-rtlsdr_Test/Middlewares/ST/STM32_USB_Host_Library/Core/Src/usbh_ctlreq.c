/**
  ******************************************************************************
  * @file    usbh_ctlreq.c 
  * @author  MCD Application Team / Victor Pecanins <vpecanins@gmail.com>
  * @version V3.2.2 (Modified for Stalled transfer handling)
  * @date    07-July-2015 (Modified 25/09/2016)
  * @brief   This file implements the control requests for device enumeration
  ******************************************************************************
  * @attention
  *
  * This file is part of STM32 Cube package. The control transfer parts were
  * modified by Victor Pecanins <vpecanins@gmail.com> to handle stalled control
  * transfers, resetting the pipes. On the original code, the STALLED status was
  * not handled at all, probably because of a bug or maybe it was correct but
  * I didn't understood it. However, this modification shows the communications
  * can be recovered if a transfer fails in STALLED, for example, when an I2C 
  * write by the RTL2832 is not ACK'ed by the peripheral.
  * 
...
static void USBH_Wdt0(int s) {
  if (s == usbh_wdt0_s) {
    if (usbh_wdt0_n == 1000) {
      USBH_DbgLog("CtrlReq stopped at state: %d", s);
      usbh_wdt0_n = 0;
      while (1) {
...
static void USBH_Wdt(int s) {
  if (s == usbh_wdt_s) {
    if (usbh_wdt_n == 1000) {
      USBH_DbgLog("Stopped at state: %d", s);
      usbh_wdt_n = 0;
      while (1) {
...
static USBH_StatusTypeDef USBH_HandleControl (USBH_HandleTypeDef *phost)
{
  ...
  case CTRL_DATA_IN_WAIT:
    ...
    { 
      ...
      phost->Control.state = CTRL_STALLED; /* vpecanins LINE */
#if (USBH_USE_OS == 1)
    osMessagePut ( phost->os_event, USBH_CONTROL_EVENT, 0);
#endif      
    }   
  ...
  case CTRL_ERROR:
    ...
    accepted.
    */
    USBH_DbgLog("Single control error");
    ...
    else
    {
      ...
      USBH_DbgLog("Control error");
      status = USBH_FAIL;
    }
    break;
    
  /* vpecanins BEGIN */
  /* This modification was added to reset control pipes in case of
   * stalled transfer. It is necessary to do the "tuner probe" routine
   * 
   * Apparently, ST original implementation of control transfers is 
   * somehow minimal and did not include this, as well as some state
   * transitions at the states CTRL_STATUS_IN_WAIT, CTRL_DATA_IN_WAIT */
  ...
  case CTRL_STALLED:
    ...
    // Reset control pipes on stalled condition 
    
    // Close control in & out pipes
    USBH_ClosePipe (phost, phost->Control.pipe_in);
    USBH_ClosePipe (phost, phost->Control.pipe_out);
    
    // Open control in & out pipes again 
    USBH_OpenPipe (phost,
                         phost->Control.pipe_in,
                         0x80,
    ...
    //USBH_DbgLog("Stalled control transfer");
  break;
  /* vpecanins END */
