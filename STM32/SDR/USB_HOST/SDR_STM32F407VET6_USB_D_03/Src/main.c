...
int main(void)
{
 ...
 //Start_TFT();// Test TFT
	  ...
	  /* Init Host Library */
	// USBH_Init(&hUsbHostFS, USBH_UserProcess, 0);   // Инициализация USB порта
	 /* Add Supported Class */
	  
   //USBH_RegisterClass(&hUsbHostFS, USBH_RTLSDR_CLASS); // Подключение класса USBH_RTLSDR_CLASS

	 /* Start Host Process */
 //  USBH_Start(&hUsbHostFS);             //
	
	 RTLSDR_HandleTypeDef *RTLSDR_Handle = (RTLSDR_HandleTypeDef*) hUsbHostFS.pActiveClass->pData; //!! Проверить результат работы этой команды.
...
//    default:
//    printf("**Unknown USBH User State: %d", id);
//      break; 
//    }
//  }
//  id_prev = id;
