...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->gState)
  {
    case HOST_IDLE :
      ...
      if ((phost->device.is_connected) != 0U)
      {
        ...
        /* Wait for 200 ms after connection */
				/* Подождите 200 мс после подключения */
        phost->gState = HOST_DEV_WAIT_FOR_ATTACHMENT;
        USBH_Delay(200U);
        (void)USBH_LL_ResetPort(phost);

        /* Make sure to start with Default address */
				/* Обязательно начните с адреса по умолчанию */
        phost->device.address = USBH_ADDRESS_DEFAULT;
        phost->Timeout = 0U;
    ...
    case HOST_DEV_WAIT_FOR_ATTACHMENT: /* Wait for Port Enabled Ждем включения порта */
      ...
      if (status == USBH_OK)
      {
        ...
        else
        {
          phost->gState = HOST_INPUT;
					USBH_UsrLog("This device HOST_INPUT.");
        }
#if (USBH_USE_OS == 1U)
    ...
    case HOST_INPUT:
    {
      /* user callback for end of device basic enumeration */
			/* обратный вызов пользователя для завершения базового перечисления устройства */
      if (phost->pUser != NULL)
      {
        phost->pUser(phost, HOST_USER_SELECT_CONFIGURATION);
				USBH_UsrLog("** phost->pUser %d",(char)HOST_USER_SELECT_CONFIGURATION );
        phost->gState = HOST_SET_CONFIGURATION;
