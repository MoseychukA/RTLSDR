...
/**
  ...
  * @retval USBH Status
   * Этот запрос используется для очистки или отключения определенной функции.
   * @param phost: дескриптор хоста
   * @param ep_num: номер конечной точки
   * @param hc_num: номер хост-канала.
   * @retval Статус USBH
  */
...
static USBH_StatusTypeDef USBH_HandleControl(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->Control.state)
  {
    ...
    case CTRL_ERROR:
      ...
      accepted.
		   После возникновения условия остановки или обнаружения ошибки
       хосту, конечной точке управления разрешено восстановиться, приняв следующую установку.
       ПИД; т.е. для управления не требуются действия по восстановлению через какой-то другой канал
       конечные точки. Для канала управления по умолчанию сброс устройства в конечном итоге будет
       требуется для устранения состояния остановки или ошибки, если следующий PID настройки не
       принял.
