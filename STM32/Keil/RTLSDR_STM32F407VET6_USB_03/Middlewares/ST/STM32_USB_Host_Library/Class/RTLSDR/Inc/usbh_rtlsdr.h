...
/* Интерфейс тюнера RTLSDR */
/* Все модули тюнера должны реализовывать эти функции */
 ...
 *  Примечание о размерах буфера **
  * Очевидно, что все буферы должны иметь длину, кратную
  * 4 байта (блоки по 32 бита). В противном случае USB будет записывать за пределы
  * буфер и вызвать переполнение буфера. Это было физически проверено,
  * если буферы меньше 4 байт, следующая переменная внутри
  * структура изменяется. Вероятно причина в stm32f7xx_ll_usb.c*
*/
