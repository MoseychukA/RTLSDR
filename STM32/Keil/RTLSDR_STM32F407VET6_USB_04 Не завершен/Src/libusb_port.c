...
uint32_t LIBUSB_CALL libusb_get_device_list(libusb_context* ctx, libusb_device** *list) { return 0; }
...
int LIBUSB_CALL libusb_get_configuration(libusb_device_handle* dev, int* config) { return 0; }
int LIBUSB_CALL libusb_get_device_descriptor(libusb_device* dev, struct libusb_device_descriptor* desc) { return 0; }
int LIBUSB_CALL libusb_get_active_config_descriptor(libusb_device* dev,struct libusb_config_descriptor** config) { return 0; }
int LIBUSB_CALL libusb_get_config_descriptor(libusb_device* dev, uint8_t config_index, struct libusb_config_descriptor** config) { return 0; }
int LIBUSB_CALL libusb_get_config_descriptor_by_value(libusb_device* dev, uint8_t bConfigurationValue, struct libusb_config_descriptor** config) { return 0; }
...
int LIBUSB_CALL libusb_get_max_packet_size(libusb_device* dev, unsigned char endpoint) { return 0; }
int LIBUSB_CALL libusb_get_max_iso_packet_size(libusb_device* dev, unsigned char endpoint) { return 0; }

int libusb_control_transfer(libusb_device_handle* dev_handle,
                            uint8_t request_type, uint8_t bRequest, uint16_t wValue, uint16_t wIndex,
                            unsigned char* data, uint16_t wLength, unsigned int timeout) {

    hUsbHostFS.Control.setup.b.bmRequestType = request_type;
    hUsbHostFS.Control.setup.b.bRequest      = bRequest;
    hUsbHostFS.Control.setup.b.wValue.w      = wValue;
    hUsbHostFS.Control.setup.b.wIndex.w      = wIndex;
    hUsbHostFS.Control.setup.b.wLength.w     = wLength;

    int status;
    
    do {
        status = USBH_CtlReq(&hUsbHostFS, data, wLength);
    } while (status == 1);

    if (status != USBH_OK)  {
        hUsbHostFS.RequestState = CMD_SEND;
        return 0;
...
int libusb_bulk_transfer(libusb_device_handle* dev_handle,
    ...
    USBH_BulkReceiveData(&hUsbHostFS,
                         data, // rx buffer
