...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
         ...
         * @retval Номер трубы*/
				InPipe = USBH_AllocPipe(&hUsbHostFS, USB_PIPE_NUMBER);  // USB_PIPE_NUMBER 0x81 
			  printf("InPipe %d \r\n",InPipe);
				lcdSetTextFont(&Font12);
	      lcdSetTextColor(COLOR_BLACK, COLOR_WHITE);
				lcdSetCursor(10, 25);   // x,y
		    lcdPrintf("InPipe %d \r\n",InPipe) ; 
