...
{	
  ...
  interface = USBH_FindInterface(phost, 
	...
	else 
	{
		...
		/* Инициализируем автомат для записи регистров инициализации */
		...
		RTLSDR_Handle->probeState = RTLSDR_PROBE_FC0012; //RTLSDR_PROBE_R820T;
		...
		/*Собираем адрес и длину конечной точки потока выборки SDR*/
		if(phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].bEndpointAddress & 0x80) 
		{	   
			RTLSDR_Handle->CommItf.SdrEp = 	phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].bEndpointAddress;

			RTLSDR_Handle->CommItf.SdrEpSize  = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize;
		}
		...
		/*Выделяем длину номера хост-канала в*/
		RTLSDR_Handle->CommItf.SdrPipe =  USBH_AllocPipe(phost, RTLSDR_Handle->CommItf.SdrEp);
		
		/* Open pipe for SDR sample stream endpoint */
		/* Открыть канал для конечной точки потока выборки SDR */
		USBH_OpenPipe  (phost,
		...
		RTLSDR_Handle->CommItf.buff = (uint8_t*)((uint32_t)(0xC0000000 + (480 * 272 * 4)));
    ...
    RTLSDR_Handle->CommItf.buffSize = 1*512;
		...
		/* Timer3 используется для измерения реальной пропускной способности */
		__HAL_RCC_TIM3_CLK_ENABLE();

    /* Compute the prescaler value to have TIMx counter clock equal to 100000 Hz */
		/* Вычисляем значение прескалера, чтобы тактовая частота счетчика TIMx была равна 100000 Гц */
    RTLSDR_Handle->uwPrescalerValue = (uint32_t)((SystemCoreClock / 2) / 100000) - 1;

    /* Set TIMx instance */
		/* Установка экземпляра TIMx */
    ...
    if (HAL_TIM_Base_Start(&(RTLSDR_Handle->TimHandle)) != HAL_OK) 
      ...
      USBH_DbgLog("Unable to start Timer 3");
		...
		{
			USBH_DbgLog("Start Timer 3");
		}
		
	}
	
	//USBH_DbgLog("USBH_RTLSDR_InterfaceInit status %d",status);
...
//uint8_t 
USBH_StatusTypeDef RTLSDR_i2c_read_reg(USBH_HandleTypeDef *phost, uint8_t i2c_addr, uint8_t reg)
{
  RTLSDR_HandleTypeDef *RTLSDR_Handle = (RTLSDR_HandleTypeDef*) phost->pActiveClass->pData;
...
/**
  ...
  * @retval USBH Status
* @brief USBH_RTLSDR_ClassRequest
   * Функция отвечает за обработку стандартных запросов.
   * для класса RTLSDR.
...
{   
  ...
  USBH_DbgLog("USBH_RTLSDR_ClassRequest");	
  USBH_StatusTypeDef rStatus = USBH_FAIL;  
  USBH_StatusTypeDef uStatus = USBH_FAIL;
  ...
  USBH_DbgLog("RTLSDR_Handle->reqState %d",RTLSDR_Handle->reqState);	
...
{
  USBH_DbgLog("**RTLSDR_probe_tuners");
  USBH_StatusTypeDef rStatus = USBH_FAIL;  
  USBH_StatusTypeDef uStatus = USBH_FAIL;
  ...
  switch (RTLSDR_Handle->probeState) 
	{
		...
		case RTLSDR_PROBE_FC0012:
				
				USBH_DbgLog("RTLSDR_PROBE_FC0012");
        uStatus = RTLSDR_i2c_read_reg(phost, FC0012_I2C_ADDR, FC0012_CHECK_ADDR);
        if (uStatus == USBH_OK || uStatus == USBH_NOT_SUPPORTED) 
				{
          if (RTLSDR_Handle->i2cReadVal == FC0012_CHECK_VAL) 
					{
            USBH_DbgLog( "Found Fitipower FC0012 tuner");
          }
					else 
					{
            USBH_DbgLog( "FC0012 not found: %02X", RTLSDR_Handle->i2cReadVal);
          }
    ...
    case RTLSDR_PROBE_FC0013:
				 USBH_DbgLog("RTLSDR_PROBE_FC0013");
    ...
    case RTLSDR_PROBE_R820T:
			USBH_DbgLog("RTLSDR_PROBE_R820T");
    ...
    case RTLSDR_PROBE_R828D:
			USBH_DbgLog("RTLSDR_PROBE_R828D");
...
static USBH_StatusTypeDef USBH_RTLSDR_Process (USBH_HandleTypeDef *phost)
{
  ...
  switch (RTLSDR_Handle->xferState) 
		...
		case RTLSDR_XFER_COMPLETE:
			USBH_DbgLog("RTLSDR_XFER_COMPLETE");
			RTLSDR_Handle->xferState = RTLSDR_XFER_START;
			rStatus = USBH_OK;
		break;
	}
