...
/*
    ...
    Пожалуйста, добавьте сюда свое устройство и отправьте патч на адрес osmocom-sdr@lists.osmocom.org.
*/
...
int rtlsdr_set_sample_rate(rtlsdr_dev_t* dev, uint32_t samp_rate) {
    ...
    if ((samp_rate <= 225000) || (samp_rate > 3200000) ||
        ...
        USBH_DbgLog("Invalid sample rate: %u Hz\n", samp_rate);
        return -EINVAL;
    }
    ...
    { USBH_DbgLog("Exact sample rate is: %f Hz\n", real_rate); }
...
int rtlsdr_set_direct_sampling(rtlsdr_dev_t* dev, int on) {
    ...
    if (on) {
        ...
        USBH_DbgLog("Enabled direct sampling mode, input %i\n", on);
        ...
        USBH_DbgLog("Disabled direct sampling mode\n");
...
int rtlsdr_open(rtlsdr_dev_t** out_dev, uint32_t index) {
    ...
    if ((reg & 0x7f) == FC2580_CHECK_VAL) {
        USBH_DbgLog("Found FCI 2580 tuner\n");
...
found:
    ...
    switch (dev->tuner_type) {
        ...
        case RTLSDR_TUNER_UNKNOWN:
            USBH_DbgLog("No supported tuner found\n");
...
//!!    struct timeval zerotv = { 0, 0 };
    ...
    for (i = 0; i < dev->xfer_buf_num; ++i) {
        ...
        if (r < 0) {
            USBH_DbgLog("Failed to submit transfer %i!\n", i);
   ...
   //!!     r = libusb_handle_events_timeout_completed(dev->ctx, &tv, &dev->async_cancel);
        ...
        if (r < 0) {
            /*USBH_DbgLog("handle_events returned: %d\n", r);*/
