...
void init_adsb_dev()
{
    adsbdev = (class_adsb_dev*)calloc(1, sizeof(class_adsb_dev));
...
int rtlsdr_set_sample_rate(rtlsdr_dev_t* dev, uint32_t samp_rate)
{
    ...
    if (((double)samp_rate) != real_rate)  fprintf(stderr, "Exact sample rate is: %f Hz\n", real_rate);
...
int rtlsdr_read_sync(rtlsdr_dev_t* dev, void* buf, int len, int* n_read)
{
    if (!dev)
        return -1;
    fprintf(stderr, "*** rtlsdr_read_sync start.\n");

    //esp_libusb_control_transfer
