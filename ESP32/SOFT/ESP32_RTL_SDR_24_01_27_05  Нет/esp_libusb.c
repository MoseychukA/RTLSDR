...
static class_adsb_dev_lib *adsbdev;

void init_adsb_dev()
{
   //!! adsbdev = calloc(1, sizeof(class_adsb_dev_lib));
...
int esp_libusb_bulk_transfer(class_driver_lib *driver_obj, unsigned char endpoint, unsigned char *data, int length, int *transferred, unsigned int timeout)
{
...
int esp_libusb_control_transfer(class_driver_lib *driver_obj, uint8_t bm_req_type, uint8_t b_request, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength, unsigned int timeout)
{
