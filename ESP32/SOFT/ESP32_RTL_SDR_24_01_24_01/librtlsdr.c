...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint8_t index, usb_host_client_handle_t client_hdl)
{
 ...
 // ESP_ERROR_CHECK(usb_host_device_open(driver_obj->client_hdl, index, &driver_obj->dev_hdl));
  ...
  err = usb_host_device_open(driver_obj->client_hdl, index, &driver_obj->dev_hdl);

  if (err != ESP_OK)
  {
      ESP_LOGI(TAG_ADSB, "*** usb_host_device_open error: %x", err);
  }
  else
  {
      ESP_LOGI(TAG_ADSB, "*** usb_host_device_open OK!!: %x", err);
  }

  dev->driver_obj = driver_obj;
 // ESP_ERROR_CHECK(usb_host_interface_claim(dev->driver_obj->client_hdl, dev->driver_obj->dev_hdl, 0, 0));

  err = usb_host_interface_claim(dev->driver_obj->client_hdl, dev->driver_obj->dev_hdl, 0, 0);
  if (err != ESP_OK)
  {
      ESP_LOGI(TAG_ADSB, "*** usb_host_interface_claim error: %x", err);
  }
  else
  {
      ESP_LOGI(TAG_ADSB, "*** usb_host_interface_claim OK!!: %x", err);
  }
