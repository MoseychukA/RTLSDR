...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint8_t index, usb_host_client_handle_t client_hdl)
{
    ...
    ESP_LOGI("", "*** driver_obj->client_hdl : %x", driver_obj->client_hdl);

   // ESP_ERROR_CHECK(usb_host_device_open(driver_obj->client_hdl, index, &driver_obj->dev_hdl));

    esp_err_t err = usb_host_device_open(driver_obj->client_hdl, index, &driver_obj->dev_hdl);
    if (err != ESP_OK) ESP_LOGI("", "usb_host_device_open error!: %x", err);

    ESP_LOGI("", "*** usb_host_device_open Ok!");
...
int rtlsdr_read_sync(rtlsdr_dev_t *dev, void *buf, int len, int *n_read)
{
    ...
    return esp_libusb_bulk_transfer(dev->driver_obj, 0x81, /*(uint8_t*)*/(unsigned char*) buf, len, n_read, BULK_TIMEOUT);
}
