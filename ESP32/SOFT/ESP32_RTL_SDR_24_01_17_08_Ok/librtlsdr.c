...
int rtlsdr_read_sync(rtlsdr_dev_t *dev, void *buf, int len, int *n_read)
{
    if (!dev)
        return -1;
    fprintf(stderr, "*** rtlsdr_read_sync *** \n");
...
int esp_libusb_bulk_transfer(class_driver_t *driver_obj, unsigned char endpoint, unsigned char *data, int length, int *transferred, unsigned int timeout)
{
    ...
    assert(driver_obj->dev_hdl != NULL);
      ...
      /*
      * @brief Округляем до целого числа, кратного MPS конечной точки.
      * Это удобная функция округления размера/длины до MPS (максимального размера пакета) конечной точки. Это полезно
      * при расчете длины передачи или буфера конечных точек IN.
      * - Если MPS <= 0, эта функция вернет 0.
      * - Если num_bytes <= 0, эта функция вернет 0
      * @param[in] num_bytes Количество байтов
      * @param[in] mps MPS
      * @return int Округление целого числа, кратного MPS
      */
