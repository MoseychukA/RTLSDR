...
int rtlsdr_read_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
    ...
    //uint8_t* buffer = new uint8_t(out_block_size * sizeof(uint8_t));
   //!! unsigned char* data =  new unsigned char(len * sizeof(char));
    int ret = 0;//!! esp_libusb_control_transfer(dev->driver_obj, CTRL_IN, 0, addr, index, (unsigned char*)data, len, CTRL_TIMEOUT);
...
int rtlsdr_write_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
   ...
   // unsigned char* data = new unsigned char(len * sizeof(char));
    ...
    //!!data[0] = *array;
    int ret = 0;// esp_libusb_control_transfer(dev->driver_obj, CTRL_OUT, 0, addr, index, (unsigned char*)data, len, CTRL_TIMEOUT);
...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint8_t index, usb_host_client_handle_t client_hdl)
{
    ...
    //class_driver_t_lib *driver_obj = calloc(1, sizeof(class_driver_t_lib));
    //if (NULL == dev)
    //    return -ENOMEM;
...
//!!
int rtlsdr_read_sync(rtlsdr_dev_t *dev, /*void*/unsigned char *buf, int len, int *n_read)
{
    if (!dev)
        return -1;
